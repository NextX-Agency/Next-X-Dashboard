# NextX Dashboard — agent instructions
...
> **Kept in sync with `CLAUDE.md`.** Both files carry the same rules verbatim so that Codex and
> Claude cannot end up working to different standards. **If you change one, change the other.**
