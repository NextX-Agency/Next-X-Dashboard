  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div
        ...
        tabIndex={-1}
        className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-white flex flex-col overflow-hidden border-l border-neutral-200"
      >
        {/* Header */}
        <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-neutral-100">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-sm bg-[#f97015]/10 flex items-center justify-center">
              <ShoppingBag size={18} className="text-[#f97015]" />
            </div>
            <div>
              <h2 className="font-semibold text-[#111111]">Winkelwagen</h2>
          ...
          <button 
            onClick={onClose}
            className="w-10 h-10 rounded-sm hover:bg-[#f97015]/10 flex items-center justify-center transition-colors"
          >
            <X size={20} className="text-[#111111]/60" />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full py-12 px-6">
              ...
              <p className="text-sm text-[#111111]/60 text-center mb-6">
                Voeg producten toe om een bestelling te plaatsen
              </p>
              <button
                onClick={onClose}
                className="px-6 py-2.5 rounded-sm bg-[#f97015] text-white text-sm font-medium hover:bg-[#d95c08] transition-colors"
              >
                Verder winkelen
              </button>
            </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="text-xs text-[#111111]/50 font-medium px-1">
                {totalItems} {totalItems === 1 ? 'item' : 'items'} in je winkelwagen
              </div>
              
              {/* Stock warning banner */}
              {hasStockIssues && (
                <div className="flex items-center gap-2 p-3 rounded-sm bg-amber-50 border border-amber-200 text-amber-700">
              ...
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-sm bg-white overflow-hidden flex-shrink-0 border border-neutral-100 relative">
                      {item.imageUrl ? (
                        <Image
                          src={item.imageUrl}
                          alt={item.name}
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      <div>
                        <h4 className={`font-medium text-sm leading-tight line-clamp-2 ${isOutOfStock ? 'text-neutral-400' : 'text-[#111111]'}`}>
                          {item.name}
                        </h4>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className={`flex items-center rounded-sm border bg-white ${
                          isOverStock ? 'border-amber-300' : 'border-neutral-200'
                        }`}>
                          <button
                            onClick={() => onUpdateQuantity(item.id, item.quantity - 1)}
                            className="w-7 h-7 flex items-center justify-center text-[#111111]/50 hover:text-[#111111] active:bg-neutral-100 transition-colors"
                          >
                            <Minus size={12} />
                          </button>
                          <span className={`w-7 text-center text-xs font-semibold ${
                            isOverStock ? 'text-amber-600' : 'text-[#111111]'
                          }`}>
                            {item.quantity}
                          </span>
                          <button
                            ...
                            className={`w-7 h-7 flex items-center justify-center transition-colors ${
                              canIncrement
                                ? 'text-[#111111]/50 hover:text-[#111111] active:bg-neutral-100'
                        ...
                        <button
                          ...
                          className="text-[11px] text-[#111111]/40 hover:text-red-500 active:text-red-600 transition-colors px-2 py-1"
                        >
                          Verwijder
                        </button>
                      </div>
        ...
        {items.length > 0 && (
          <div className="shrink-0 border-t border-neutral-200 bg-white">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-sm bg-[#f97015] flex items-center justify-center">
              ...
              <div className={`w-8 h-8 rounded-sm bg-white border border-neutral-200 flex items-center justify-center transition-transform duration-200 ${isCheckoutExpanded ? 'rotate-180' : ''}`}>
                <ChevronDown size={18} className="text-[#111111]/60" />
              </div>
            </button>
            ...
            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isCheckoutExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
              <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full h-11 pl-10 pr-4 rounded-sm border border-neutral-200 bg-white text-sm text-[#111111] appearance-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#f97015]/30 focus:border-[#f97015]"
                      >
                        {locations.map((loc) => (
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 h-10 rounded-sm text-sm font-medium transition-colors ${
                        pickupDate === 'today'
                          ? 'bg-[#f97015] text-white'
                          : 'bg-white border border-neutral-200 text-[#111111]/70 hover:border-[#f97015]/50'
                      }`}
                    >
                      Vandaag
                    </button>
                  ...
                  {pickupDate === 'custom' && (
                    <input
                      ...
                      className="w-full h-11 px-4 rounded-sm border border-neutral-200 bg-white text-sm text-[#111111] mt-2 focus:outline-none focus:ring-2 focus:ring-[#f97015]/30 focus:border-[#f97015]"
                    />
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-[#111111]/60 mb-1.5">
                      Naam
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder="Je naam"
                      className="w-full h-11 px-4 rounded-sm border border-neutral-200 bg-white text-sm text-[#111111] placeholder:text-[#111111]/40 focus:outline-none focus:ring-2 focus:ring-[#f97015]/30 focus:border-[#f97015]"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-[#111111]/60 mb-1.5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-4 py-3 rounded-sm border border-neutral-200 bg-white text-sm text-[#111111] placeholder:text-[#111111]/40 resize-none focus:outline-none focus:ring-2 focus:ring-[#f97015]/30 focus:border-[#f97015]"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="px-4 sm:px-6 pb-4 sm:pb-6">
              <div className="flex items-center justify-between py-3 border-t border-neutral-200 mb-3">
                <span className="text-sm text-[#111111]/70">Totaal ({totalItems} items)</span>
