  ...
  return <main className="finance-control-workspace min-h-screen bg-[#090d13] text-slate-100">
    ...
    <header className="flex flex-col gap-5 border-b border-white/[0.08] pb-6 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <div className="mt-5 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500"><span className="h-1.5 w-1.5 rounded-full bg-orange-300" />Month-end workspace</div>
        <h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-white sm:text-4xl">Close only when the<br className="hidden sm:block" /> evidence agrees.</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">A controlled path from evidence review to period lock. The database repeats every gate inside the close transaction.</p>
      </div>
      <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-2"><label className="px-2 text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500" htmlFor="period-end">Period end</label><input id="period-end" aria-label="Period end" type="date" value={periodEnd} onChange={(event) => setPeriodEnd(event.target.value)} className="rounded-lg border border-white/[0.1] bg-[#0b111a] px-3 py-2 text-sm text-slate-100" /><button type="button" onClick={() => void load()} className="inline-flex items-center gap-2 rounded-lg border border-white/[0.1] bg-white/[0.05] px-3.5 py-2 text-xs font-semibold text-slate-200 transition hover:bg-white/[0.1]"><RefreshCcw size={14} className={loading ? 'animate-spin' : ''} />Refresh</button></div>
    </header>
