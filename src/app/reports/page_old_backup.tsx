...
import { BarChart3, TrendingUp, Package, DollarSign, Calendar, Award, ShoppingBag, Wallet, MapPin, Layers, Sparkles } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  const loadData = async () => {
    ...
    // Load all data
    const [allSalesRes, periodSalesRes, itemsRes, stocksRes, locationsRes, expensesRes, commissionsRes] = await Promise.all([
      supabase
        .from('sales')
        .select('*, sale_items(*)')
    ...
    if (allSalesRes.data) {
      ...
      if (itemsRes.data) {
        calculateMonthlySalesHistory(allSalesRes.data as SaleWithItems[], itemsRes.data as ItemWithCombo[])
      }
    }
    if (periodSalesRes.data) setSales(periodSalesRes.data as SaleWithItems[])
  ...
  const calculateMonthlySalesHistory = (salesData: SaleWithItems[], itemsList: ItemWithCombo[]) => {
  ...
  const calculateMonthlyExpenses = (expensesList: ExpenseWithCategory[]) => {
    ...
    expensesList.forEach(expense => {
      ...
      if (displayCurrency === 'USD') {
        amount = expense.currency === 'USD' ? expense.amount : expense.amount / rate
      } else {
        amount = expense.currency === 'SRD' ? expense.amount : expense.amount * rate
      }
